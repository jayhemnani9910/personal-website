...
@cache
def dfs_global(c0, c1, c2, c3, c4, c5, non_dom):
    best = 0
    for o0, o1, o2, o3, o4, o5, sav in non_dom:
        if c0 >= o0 and c1 >= o1 and c2 >= o2 and c3 >= o3 and c4 >= o4 and c5 >= o5:
            cur = sav + dfs_global(c0-o0, c1-o1, c2-o2, c3-o3, c4-o4, c5-o5, non_dom)
            if cur > best: best = cur
    return best

class SolutionGlobal:
    def shoppingOffers(self, price: List[int], special: List[List[int]], needs: List[int]) -> int:
        n = len(price)
        if n == 6:
            p0, p1, p2, p3, p4, p5 = price
        ...
        initial_cost = n0*p0 + n1*p1 + n2*p2 + n3*p3 + n4*p4 + n5*p5
        return initial_cost - dfs_global(n0, n1, n2, n3, n4, n5, tuple(non_dominated))
...
dfs_global.cache_clear()
start = time.time()
for p, s, n in testcases:
    sol_glob.shoppingOffers(p, s, n)
end = time.time()
print(f"Global DFS: {(end - start)*1000:.2f} ms")
