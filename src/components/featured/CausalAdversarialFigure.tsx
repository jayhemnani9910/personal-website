...
function color(n: Node) {
  if (n.accent === "red") return "var(--accent)";
  if (n.accent === "blue") return "var(--ink)";
  return "var(--ink)";
}
...
export function CausalAdversarialFigure({ className }: { className?: string }) {
  ...
  return (
    ...
    >
      ...
      {/* flow edges */}
      <g stroke="var(--ink-mute)" strokeWidth={1.25} markerEnd="url(#cag-arrow)">
      ...
      <path
        ...
        stroke="var(--ink-faint)" strokeWidth={1} strokeDasharray="4 3" markerEnd="url(#cag-arrow)"
      />
      <text x={NODES.selector.x + NODES.selector.w / 2 + 8} y={296} fontSize={9} fill="var(--ink-mute)" letterSpacing="0.06em">auditor · depth/loop caps</text>

      {/* nodes */}
      {Object.entries(NODES).map(([key, n]) => (
        <g key={key}>
          <rect x={n.x} y={n.y} width={n.w} height={n.h} rx={4} fill="var(--paper)" stroke={color(n)} strokeWidth={n.accent ? 1.75 : 1.5} />
          <text x={n.x + n.w / 2} y={n.sub ? n.y + 21 : n.y + n.h / 2 + 4} textAnchor="middle" fontSize={11} fill={color(n)} fontWeight={600}>{n.label}</text>
          {n.sub && <text x={n.x + n.w / 2} y={n.y + 35} textAnchor="middle" fontSize={9} fill="var(--ink-mute)">{n.sub}</text>}
        </g>
      ))}

      {/* verdict chip */}
      <g>
        <rect x={400} y={18} width={150} height={22} rx={11} fill="var(--rule-soft)" stroke="var(--rule)" strokeWidth={1} />
        <text x={475} y={33} textAnchor="middle" fontSize={9.5} fill="var(--ink)" letterSpacing="0.04em">VERIFIED · FALSIFIED</text>
      </g>
    </svg>
  );
}
