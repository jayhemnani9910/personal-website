...
summary: "Multi-model CV pipeline for soccer video analysis with YOLOv8n detection, ByteTrack tracking, and GraphSAGE interaction classification. Full MLOps with DVC, MLflow, and FastAPI."
role: "Builder"
period: "2024"
...
challenge: "Build an end-to-end pipeline from raw soccer video to tactical interaction graphs, with full MLOps infrastructure for experiment tracking, data versioning, and model deployment."
solution:
  - "7-stage DVC pipeline: data fetch → preprocess → YOLOv8n detection → ByteTrack tracking → graph construction → GraphSAGE classification → model registry."
  - "FastAPI REST service with endpoints for detection, tracking, and YouTube highlight processing."
  - "MLflow experiment tracking across 50+ training runs with automated model versioning via DVC."
impact:
  - "Complete MLOps stack with DVC pipeline orchestration, MLflow tracking, and TensorRT export."
  - "Processes YouTube highlights and live RTSP streams via FastAPI endpoints."
  - "GraphSAGE neural network classifies player interactions from spatial proximity graphs."
featured: true
priority: 1
deepDive:
...
---
# Overview
Multi-model computer vision pipeline for soccer video analysis. Uses YOLOv8n for detection, ByteTrack for multi-object tracking with Kalman filtering, and GraphSAGE for interaction classification from spatial proximity graphs. Full MLOps infrastructure with a 7-stage DVC pipeline, MLflow experiment tracking, FastAPI REST endpoints, and Docker containerization.
